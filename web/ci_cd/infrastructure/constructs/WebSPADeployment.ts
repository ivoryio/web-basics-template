...
export namespace WebSPA {
  export interface DeploymentDNSProps {
    recordName: string
    hostedZoneId: string
  ...
  export interface DeploymentProps extends cdk.StackProps {
    dnsInfo: DeploymentDNSProps
  }

  export class DeploymentEnvironment extends cdk.Construct {
    ...
    private createCDNDistribution(deploymentBucket: s3.IBucket, dnsInfo: DeploymentDNSProps) {
    ...
    private createDnsAlias(dnsInfo: DeploymentDNSProps, distibution: cf.CloudFrontWebDistribution) {
      const props = {
